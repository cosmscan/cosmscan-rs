...
impl Client {
    ...
    pub async fn new(endpoint: String) -> Result<Self, Error> {
        let tm_client =
            tendermint_rpc::HttpClient::new(endpoint.as_str()).map_err(|e| Error::from(e))?;
        let grpc_client = service_client::ServiceClient::connect(endpoint.clone())
            .await
    ...
    pub async fn get_transaction(
        &mut self,
        hash: String,
    ) -> Result<cosmos_sdk_proto::cosmos::tx::v1beta1::GetTxResponse, Error> {
