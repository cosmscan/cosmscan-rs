...
impl Fetcher {
    ...
    pub fn new(
        ...
        let tendermint_client =
            tendermint_rpc::HttpClient::new(config.fetcher.tendermint_rpc.as_str())
                .map(Arc::<tendermint_rpc::HttpClient>::new)
                .map_err(|e| Error::from(xe))?;
    ...
    pub async fn run(&self) -> Result<(), Error>{
        ...
        loop {
            ...
            let block_height = tendermint::block::Height::from(current_block);
            let resp = self.tendermint_client.block(block_height).await?;
            info!("fetcher listens block_number:{}, hash: {}", resp.block.header.height, resp.block.header.hash());
            ...
            self.journal[current_block] = MsgCommittedBlock {
